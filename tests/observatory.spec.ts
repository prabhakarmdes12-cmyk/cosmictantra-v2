...
import { getCelestialDetail, getConstellationDetail, getStarDetail, parseCelestialSelection, PLANET_DETAILS } from '../src/lib/astronomy/celestialCatalog';
...
import { createLocalStudyContext } from '../src/lib/observatory/studyContext';
import { createStudySnapshot, parseStudySnapshot, STUDY_QUALIFICATION_BLOCKERS } from '../src/lib/observatory/studySnapshot';
...
test.describe('Observatory coordinate and ephemeris invariants', () => {
  ...
  test('detail deep links accept known planets, stars and constellations only', () => {
    ...
    expect(parseCelestialSelection('Sirius', 'star')).toEqual({ kind: 'star', id: 'sirius' });
    expect(getStarDetail('sirius')?.astronomy).toContain('J2000');
    expect(getCelestialDetail({ kind: 'star', id: 'sirius' }).displayName).toBe('Sirius');
    expect(parseCelestialSelection('not-a-body', 'planet')).toBeNull();
    expect(parseCelestialSelection('not-a-star', 'star')).toBeNull();
  ...
  test('study snapshots preserve target, local calculation, provider provenance and qualification blockers', () => {
    const observer = { latitude: 25.3176, longitude: 82.9739 };
    const target = normalizeLiveTarget('planet', 'Sun')!;
    const localCalculation = createLocalStudyContext(target, instant, observer);
    const response = createLiveObservationResponse(
      target,
      instant.toISOString(),
      buildNasaSdoFallbackFrame(target, instant.toISOString(), '2026-08-25T00:03:00.000Z'),
      ['Helioviewer unavailable; browse fallback retained without capture time.'],
    );
    const snapshot = createStudySnapshot({
      savedAtUtc: '2026-08-25T00:04:00.000Z',
      observedAtUtc: instant.toISOString(),
      target,
      city: { id: 'varanasi', name: 'Varanasi', latitude: observer.latitude, longitude: observer.longitude, timezoneOffsetHours: 5.5 },
      localCalculation,
      liveResponse: response,
    });
    expect(snapshot.target).toEqual(target);
    expect(snapshot.localCalculation.frame).toContain('tropical');
    expect(snapshot.providerCheck.checked).toBe(true);
    expect(snapshot.providerCheck.notices[0]).toContain('Helioviewer');
    expect(snapshot.frame?.provider).toBe('nasa-sdo');
    expect(snapshot.frame?.capturedAtUtc).toBeNull();
    expect(snapshot.frame?.receivedAtUtc).toBe('2026-08-25T00:03:00.000Z');
    expect(snapshot.qualification.status).toBe('CONDITIONAL PASS');
    expect(snapshot.qualification.blockers).toEqual([...STUDY_QUALIFICATION_BLOCKERS]);
    expect(parseStudySnapshot(JSON.parse(JSON.stringify(snapshot)))).toEqual(snapshot);

    const starTarget = normalizeLiveTarget('star', 'Sirius')!;
    const starContext = createLocalStudyContext(starTarget, instant, observer);
    expect(starContext.physicalSky).toBe(true);
    expect(starContext.coordinates.altitudeDeg).not.toBeNull();
    expect(starContext.coordinates.tropicalLongitudeDeg).toBeNull();

    const localOnly = createStudySnapshot({
      savedAtUtc: '2026-08-25T00:05:00.000Z',
      observedAtUtc: instant.toISOString(),
      target: starTarget,
      city: { id: 'varanasi', name: 'Varanasi', latitude: observer.latitude, longitude: observer.longitude, timezoneOffsetHours: 5.5 },
      localCalculation: starContext,
      liveResponse: null,
    });
    expect(localOnly.providerCheck.checked).toBe(false);
    expect(localOnly.frame).toBeNull();
    expect(parseStudySnapshot({ ...snapshot, frame: { ...snapshot.frame!, frameId: '' } })).toBeNull();

    const sunPlan = planObservation(instant, observer, 'Sun');
    const logged = createObservationLogEntry({
      observedAt: instant.toISOString(),
      cityId: 'varanasi',
      cityName: 'Varanasi',
      observer,
      timezoneOffsetHours: 5.5,
      body: 'Sun',
      source: sunPlan.body.source,
      physicalSky: true,
      altitudeDeg: sunPlan.horizontal?.altitudeDeg ?? null,
      azimuthDeg: sunPlan.horizontal?.azimuthDeg ?? null,
      direction: sunPlan.direction,
      altitudeBand: sunPlan.altitudeBand,
      tropicalLongitude: sunPlan.body.tropicalLongitude,
      siderealLongitude: sunPlan.body.siderealLongitude,
      rashi: localCalculation.coordinates.rashi || 'unknown',
      nakshatra: localCalculation.coordinates.nakshatra || 'unknown',
      pada: localCalculation.coordinates.pada || 1,
      lunarSeparationDeg: null,
      moonPhase: 'New Moon',
      status: 'observed',
      note: 'Provider-aware snapshot',
      snapshot,
    }, 'snapshot-entry', '2026-08-25T00:04:00.000Z');
    const restoredEntry = parseObservationLog(serializeObservationLog([logged]))[0];
    expect(restoredEntry?.snapshot?.frame?.provider).toBe('nasa-sdo');
    expect(observationLogToCsv([logged])).toContain('targetKind');
    expect(observationLogToCsv([logged])).toContain('nasa-sdo');
    expect(observationLogToCsv([logged])).toContain('BLOCKER-1');
  });
