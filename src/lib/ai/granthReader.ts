 ...
 */
import { ValidatedScriptureEntry, VERIFIED_SCRIPTURE_CORPUS } from './scriptureCorpus';

export interface ReadRequest {
  grantha: string;         // 'gita', 'ramcharitmanas', 'madhurashtakam', etc.
  mode: 'full' | 'chapter' | 'verse' | 'section' | 'condition';
  chapter?: number;
  verse?: number;
  sectionId?: string;
  condition?: string;      // emotional/life situation for conditional read
}
...
export function readScriptureText(req: ReadRequest): ReadResponse {
  const gitaKey = (ch?: number, v?: number) => ch !== undefined && v !== undefined ? `BG_${ch}_${v}` : null;
  ...
  if (req.mode === 'verse' || req.mode === 'full' || req.mode === 'chapter') {
    if (key && VERIFIED_SCRIPTURE_CORPUS[key]) {
      const entry = VERIFIED_SCRIPTURE_CORPUS[key];
      return {
        found: true,
