...
import { parseStudySnapshot, type ObservatoryStudySnapshot } from '@/lib/observatory/studySnapshot';
...
export interface ObservationLogEntry {
  ...
  note: string;
  /** Optional v1 study envelope; older local entries remain valid without it. */
  snapshot?: ObservatoryStudySnapshot;
}
...
function normalizeEntry(value: unknown): ObservationLogEntry | null {
  ...
  const snapshot = candidate.snapshot === undefined ? undefined : parseStudySnapshot(candidate.snapshot);
...
export function observationLogToCsv(entries: ObservationLogEntry[]): string {
  const header = [
    ...
    'targetKind', 'targetId', 'targetLabel', 'snapshotSchemaVersion', 'localFrame', 'localModel',
    'localQuality', 'providerCheck', 'providerNotices', 'frameProvider', 'frameId', 'frameMode',
    'frameStatus', 'requestedAtUtc', 'capturedAtUtc', 'receivedAtUtc', 'wavelength', 'filter',
    'exposureSeconds', 'processingLevel', 'frameQuality', 'freshness', 'sourceUrl', 'attribution',
    'license', 'useNotes', 'qualificationStatus', 'qualificationBlockers',
  ];
  const rows = entries.slice(0, MAX_OBSERVATION_LOG_ENTRIES).map(entry => {
    ...
    const targetIsPhysicalBody = snapshot?.target.kind === 'planet' && snapshot.localCalculation.physicalSky;
    return [
      entry.observedAt,
      entry.cityName,
      entry.observer.latitude,
      entry.observer.longitude,
      snapshot?.target.label ?? entry.body,
      entry.status,
      snapshot?.localCalculation.physicalSky ?? entry.physicalSky,
      snapshot ? coordinates?.altitudeDeg ?? null : entry.altitudeDeg,
      snapshot ? coordinates?.azimuthDeg ?? null : entry.azimuthDeg,
      snapshot ? coordinates?.direction ?? null : entry.direction,
      snapshot ? coordinates?.altitudeBand ?? null : entry.altitudeBand,
      snapshot ? coordinates?.tropicalLongitudeDeg ?? null : entry.tropicalLongitude,
      snapshot ? coordinates?.siderealLongitudeDeg ?? null : entry.siderealLongitude,
      snapshot ? coordinates?.rashi ?? null : entry.rashi,
      snapshot ? coordinates?.nakshatra ?? null : entry.nakshatra,
      snapshot ? coordinates?.pada ?? null : entry.pada,
      targetIsPhysicalBody ? entry.lunarSeparationDeg : snapshot ? null : entry.lunarSeparationDeg,
      targetIsPhysicalBody ? entry.moonPhase : snapshot ? null : entry.moonPhase,
      entry.note,
      snapshot?.target.kind ?? null,
      snapshot?.target.id ?? null,
      snapshot?.target.label ?? null,
      snapshot?.schemaVersion ?? null,
      snapshot?.localCalculation.frame ?? null,
      snapshot?.localCalculation.model ?? null,
      snapshot?.localCalculation.quality ?? null,
      snapshot?.providerCheck.checked ?? null,
      snapshot?.providerCheck.notices.join(' | ') ?? null,
      frame?.provider ?? null,
      frame?.frameId ?? null,
      frame?.mode ?? null,
      frame?.status ?? null,
      frame?.requestedAtUtc ?? null,
      frame?.capturedAtUtc ?? null,
      frame?.receivedAtUtc ?? null,
      frame ? [frame.wavelengthLabel, frame.wavelengthNm === null ? null : `${frame.wavelengthNm} nm`].filter(Boolean).join(' · ') : null,
      frame?.filter ?? null,
      frame?.exposureSeconds ?? null,
      frame?.processingLevel ?? null,
      frame?.quality ?? null,
      frame?.freshness ?? null,
      frame?.sourceUrl ?? null,
      frame?.attribution ?? null,
      frame?.license ?? null,
      frame?.useNotes ?? null,
      snapshot?.qualification.status ?? null,
      snapshot?.qualification.blockers.join(' | ') ?? null,
