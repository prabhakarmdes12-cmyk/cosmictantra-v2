...
const ENGINE_BODIES: Partial<Record<SupportedBody, Body>> = {
  Sun: Body.Sun, Moon: Body.Moon, Mars: Body.Mars,
  Mercury: Body.Mercury, Jupiter: Body.Jupiter, Venus: Body.Venus, Saturn: Body.Saturn,
};
function localDateTime(time: ObservatoryTime) {
  const [date, clock] = time.userLocalTime.split('T');
  return { date, clock: clock.slice(0, 5) };
}
/**
 * Astronomy truth comes from the pinned MIT Astronomy Engine.
 * Canonical Jyotish truth remains a separately-labelled output from the
 * protected CosmicTantra engine.
 *
 * Rahu and Ketu (lunar nodes): no astronomy-engine body exists.
 * Canonical engine computes them analytically. Derived scientific values
 * are not available for the nodes.
 */
export function calculateCanonicalBody(body: SupportedBody, time: ObservatoryTime): BodyPosition {
  const { date, clock } = localDateTime(time);
  const offsetHours = time.timezoneOffsetMinutes / 60;
  ...
  if (engineBody) {
    // Physical planets: get tropical from astronomy-engine
    const instant = new Date(time.utcInstant);
    const geo = Ecliptic(GeoVector(engineBody, instant, true));
    tropical = geo.elon;
    ...
    scientific = {
      ...
      eclipticLatitude: geo.elat,
    };
  } else {
    // Rahu / Ketu: no astronomy-engine body.
    // Compute tropical as sidereal + ayanamsha (same convention as physical planets).
    tropical = normalizeAngle(canonicalSidereal + getLahiriAyanamsha(time.julianDate));
  }
  ...
  return {
    ...
    siderealLongitude: {
      value: canonicalSidereal, unit: 'degree',
      frame: 'ecliptic-sidereal-lahiri',
      source: 'CosmicTantra canonical Jyotish engine',
      algorithmVersion: 'cosmictantra-canonical-v34', calculatedAt,
    },
    ayanamsha: {
      value: canonicalAyanamsha, unit: 'degree', frame: 'ecliptic-tropical',
      source: 'CosmicTantra canonical Jyotish engine',
      algorithmVersion: 'cosmictantra-canonical-v34', calculatedAt,
    },
    rashi: chart.planets[body].rasiName,
    degreeInRashi: canonicalSidereal % 30,
    nakshatra: chart.planets[body].nakshatra,
    scientific,
    crossEngine: {
      astronomyDerivedSidereal: astronomyLahiri.longitude,
      canonicalSidereal,
      differenceDegrees: discrepancy,
      agreement: Math.abs(discrepancy) <= 0.05,
    },
  };
}
