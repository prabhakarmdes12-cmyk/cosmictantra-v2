/**
 * CosmicTantra V34 — Advanced Panchang & My Days Energy Engine
 * Calculates Tithi, Nakshatra, Nitya Yoga, Karana, Vara, Rahu Kala,
 * Tara Bala (9 Stars), Chandra Bala, and Personal Power/Caution Days.
 */
...
export const TARA_BALA_MAP = {
  1: { name: 'Janma', status: 'NEUTRAL', desc: 'Mind & Body active. Reflection required.' },
  2: { name: 'Sampat', status: 'POWER', desc: 'Wealth, gain, and prosperity. 🌟 POWER DAY' },
  3: { name: 'Vipat', status: 'CAUTION', desc: 'Obstacles or unexpected delays. ⚠ CAUTION DAY' },
  4: { name: 'Kshema', status: 'POWER', desc: 'Wellbeing, comfort, and safety. 🌟 POWER DAY' },
  5: { name: 'Pratyak', status: 'CAUTION', desc: 'Friction or resistance. Avoid arguments. ⚠ CAUTION DAY' },
  6: { name: 'Sadhana', status: 'POWER', desc: 'Achievement, fulfillment & success. 🌟 POWER DAY' },
  7: { name: 'Naidhana', status: 'CAUTION', desc: 'Vadha/Loss risk. High sensitivity. ⚠ CAUTION DAY' },
  8: { name: 'Mitra', status: 'POWER', desc: 'Harmony, friendship, & progress. 🌟 POWER DAY' },
  9: { name: 'Parama Mitra', status: 'POWER', desc: 'Supreme alliance & divine grace. 🌟 POWER DAY' },
};
...
export function calculatePanchang(date = new Date(), latitude = 25.5941, longitude = 85.1376, tzOffset = 5.5, birthNakshatraIndex = null, birthRasiIndex = null) {
  ...
  // Personal Energy Calculations (Tara Bala & Chandra Bala)
  let personalEnergy = null;
  if (birthNakshatraIndex !== null) {
    const count = ((moonNakIdx - birthNakshatraIndex + 27) % 27) + 1;
    const taraNum = ((count - 1) % 9) + 1;
    const taraInfo = TARA_BALA_MAP[taraNum] || TARA_BALA_MAP[1];

    let chandraHouse = null;
    if (birthRasiIndex !== null) {
      chandraHouse = ((moonRasiIdx - birthRasiIndex + 12) % 12) + 1;
    }

    const isPower = taraInfo.status === 'POWER' && (chandraHouse === null || [1, 3, 6, 7, 10, 11].includes(chandraHouse));
    ...
    personalEnergy = {
      ...
      chandraHouse,
      isPowerDay: isPower,
      isCautionDay: isCaution,
      badgeText: isPower ? '🌟 POWER DAY' : isCaution ? '⚠ CAUTION DAY' : '☽ BALANCED DAY',
    };
  }
