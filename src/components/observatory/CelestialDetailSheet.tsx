...
export default function CelestialDetailSheet({ selection, date, observer, cityId = 'varanasi', cityName, onClose }: CelestialDetailSheetProps) {
  ...
  const selectedStar = selection.kind === 'star' ? STARS.find(star => star.id === selection.id) || null : null;
  const selectedStarPoint = selectedStar ? projectStar(selectedStar, dateValue, observer, 600, 600) : null;
  ...
  const skyPosition = body
    ? equatorialToHorizontal({ raHours: body.rightAscensionHours, decDeg: body.declinationDeg }, dateValue, observer)
    : selectedStarPoint;
  ...
  const provenance = selection.kind === 'planet' && localProvenance
    ...
    : {
        label: selection.kind === 'star' ? 'Catalogue star · J2000 anchor' : 'Catalogue pattern · J2000 bright-star coordinates',
        quality: selection.kind === 'star' ? 'Catalogue projection · approximate precession' : 'Schematic pattern · approximate precession',
        ...
        note: selection.kind === 'star'
          ? 'This star is projected from a fixed bright-star catalogue with approximate precession. It is not a camera frame or a precision astrometric solution.'
          : 'Star positions are projected from a fixed bright-star catalogue with approximate precession. The pattern drawing is schematic, not a surveyed boundary or an agency image.',
      };
  ...
  const copyPosition = async () => {
    if ((!body && !selectedStar) || !navigator.clipboard) return;
    try {
      const text = body
        ? `${body.body}: tropical ${longitude(body.tropicalLongitude)}, sidereal ${longitude(body.siderealLongitude)}`
        : `${selectedStar?.name}: J2000 RA ${selectedStar?.raHours.toFixed(4)}h, Dec ${selectedStar?.decDeg.toFixed(4)}°, magnitude ${selectedStar?.magnitude.toFixed(2)}`;
      ...
      <section ref={dialogRef} role="dialog" aria-modal="true" aria-labelledby="celestial-detail-title" aria-describedby="celestial-detail-description" className="celestial-sheet-panel max-h-[92vh] w-full max-w-2xl overflow-y-auto rounded-t-3xl border border-white/10 bg-[#080B16] text-[#ECEAF1] shadow-2xl sm:h-full sm:max-h-none sm:rounded-none sm:rounded-l-3xl">
        ...
        <div className="space-y-6 p-5 sm:p-7">
          ...
          {body ? (
            ...
            selectedStar ? (
              <section className="grid grid-cols-2 gap-2 sm:grid-cols-4"><div className="rounded-xl border border-white/[0.08] bg-[#0D1222] p-3"><div className="font-mono-data text-[9px] uppercase tracking-wider text-[#8993B0]">Magnitude</div><div className="mt-1 font-mono-data text-lg font-bold text-[#F2C65D]">{selectedStar.magnitude.toFixed(2)}</div></div><div className="rounded-xl border border-white/[0.08] bg-[#0D1222] p-3"><div className="font-mono-data text-[9px] uppercase tracking-wider text-[#8993B0]">Altitude</div><div className="mt-1 font-mono-data text-lg font-bold text-[#E3E7F5]">{selectedStarPoint ? `${selectedStarPoint.altitudeDeg.toFixed(1)}°` : 'not supplied'}</div></div><div className="rounded-xl border border-white/[0.08] bg-[#0D1222] p-3"><div className="font-mono-data text-[9px] uppercase tracking-wider text-[#8993B0]">Azimuth</div><div className="mt-1 font-mono-data text-lg font-bold text-[#E3E7F5]">{selectedStarPoint ? `${selectedStarPoint.azimuthDeg.toFixed(1)}°` : 'not supplied'}</div></div><div className="rounded-xl border border-white/[0.08] bg-[#0D1222] p-3"><div className="font-mono-data text-[9px] uppercase tracking-wider text-[#8993B0]">Pattern</div><div className="mt-1 font-mono-data text-sm font-bold text-[#E3E7F5]">{constellationDisplayName(selectedStar.constellation)}</div></div></section>
            ) : (
