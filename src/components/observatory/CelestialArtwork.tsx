...
function ConstellationArtwork({ id }: { id: string }) {
  ...
  return (
    <svg viewBox="0 0 800 460" className="block h-full w-full" role="img" aria-label={`Detailed ${constellationDisplayName(id)} constellation sky map`}>
      <defs>
      ...
      {source.map(star => { const point = map.get(star.id); if (!point) return null; const radius = Math.max(3, starRadiusFromMagnitude(star.magnitude) + 1); return <g key={star.id}><circle cx={point.x} cy={point.y} r={radius + 7} fill="#B9C7FF" opacity="0.18" filter={`url(#constellation-glow-${safeId(id)})`} /><circle cx={point.x} cy={point.y} r={radius} fill={starColorFromBV(star.bv)} stroke="#FFFFFF" strokeWidth="0.8" /><text x={point.x + 10} y={point.y - 9} fill="#E9ECFF" fontFamily="JetBrains Mono, monospace" fontSize="10">{star.name}</text></g>; })}
...
export default function CelestialArtwork({ selection, className = '' }: CelestialArtworkProps) {
  return <div className={`aspect-[16/9] overflow-hidden rounded-xl border border-white/10 bg-[#050711] ${className}`}>{selection.kind === 'planet' ? <PlanetArtwork body={selection.id} /> : <ConstellationArtwork id={selection.id} />}</div>;
}
