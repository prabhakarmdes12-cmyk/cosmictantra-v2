...
export default function ObservatoryExperience() {
 ...
 return <main className="obs-page">
  ...
  <section className="obs-instrument">
   ...
   <article className="obs-inspector"><div className="obs-object"><span className="obs-object-glyph">☾</span><div><p>CHANDRA <em>चन्द्र</em></p><h2>{moon.nakshatra.name} <small>• Pada {moon.nakshatra.pada}</small></h2></div></div><div className="obs-reading"><div><label>SIDEREAL — LAHIRI</label><b>{dms(moon.siderealLongitude.value)}</b><span>{moon.rashi} · {dms(moon.degreeInRashi)}</span></div><div><label>ASTRONOMICAL / TROPICAL</label><b>{dms(moon.tropicalLongitude.value)}</b><span>of-date · Astronomy Engine</span></div><div><label>AYANAMSHA</label><b>{dms(moon.ayanamsha.value)}</b><span>Canonical Lahiri adapter</span></div></div>{moon.scientific && <div className="obs-science">ALT {moon.scientific.altitude.toFixed(2)}° · AZ {moon.scientific.azimuth.toFixed(2)}° · RA {moon.scientific.rightAscensionHours.toFixed(4)}h · DEC {moon.scientific.declination.toFixed(2)}°</div>}{!moon.crossEngine.agreement && <div className="obs-discrepancy">DEVELOPMENT: astronomical Lahiri comparison Δ {moon.crossEngine.differenceDegrees.toFixed(3)}°. Canonical Jyotish output remains shown.</div>}<div className="obs-actions"><Link href={`/panchang/dhanbad?observatory=moon&time=${encodeURIComponent(time.utcInstant)}`}>VIEW IN PANCHANG</Link><Link href="/?open=kundali-section">VIEW IN KUNDALI</Link></div></article>
  </section>
