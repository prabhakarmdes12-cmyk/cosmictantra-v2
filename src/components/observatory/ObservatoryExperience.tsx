...
export default function ObservatoryExperience() {
 const [city, setCity] = useState(DEFAULT_CITY); const [instant, setInstant] = useState(() => new Date()); const [live, setLive] = useState(true); const [showNakshatras, setShowNakshatras] = useState(true); const [details, setDetails] = useState(false);
 ...
 return <main className="obs-page">
  ...
  <section className="obs-sky" aria-label={`Computed ecliptic sky above ${observer.name}`}>
    ...
    <button className="obs-moon" style={{ left: `${16 + moon.siderealLongitude.value/360*68}%`, top: `${43 + Math.sin(moon.siderealLongitude.value*Math.PI/180)*10}%`}} aria-label="Moon selected"><Moon fill="currentColor" size={30}/><b>☾</b></button>
