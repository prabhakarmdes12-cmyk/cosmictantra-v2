...
### Study snapshot boundary

The `/observatory` Study Cockpit joins the local and provider layers for learning, but it does not merge their pixels. When a learner presses save in `ObservationLog`, `studySnapshot.ts` records a versioned envelope containing:

- the selected target, exact UTC instant, city, fixed observer coordinates and timezone offset;
- the target-aware local source path, model, frame, quality, meaningful coordinates and limitations;
- whether the provider check ran, which capabilities were returned, and all provider notices;
- the complete normalized frame provenance when available, including requested/captured/received time, wavelength/filter, exposure, processing, quality/freshness, source, attribution, license, use notes and same-origin transport pointers;
- the current `CONDITIONAL PASS` state and `BLOCKER-1`/`BLOCKER-2` references.

No image bytes are stored. JSON export keeps this structure intact; CSV export flattens the important provenance fields for classwork and analysis. The parser validates snapshots before they are rendered and retains compatibility with older notebook entries that have no snapshot envelope. This gives a learner a reproducible study record without introducing a cloud account, browser LAN access, or an implicit claim that a provider image validates the local ephemeris.
...
Automated coverage added for this seam is in `tests/observatory.spec.ts` and `tests/observatory.integration.spec.ts`:
...
- target-aware local study context for planets, nodes, stars and constellations;
- versioned snapshot creation/parsing, provider notices, frame provenance and qualification blockers;
- direct route-handler responses for local-only targets, invalid targets, locked hardware, MCP initialization and CORS preflight.
...
`npm run build` remains blocked before Next compilation when Prisma cannot retrieve its engine checksum from `binaries.prisma.sh`. Chromium also remains unavailable for full browser interaction, canvas zoom, responsive layout, and actual image-load QA; the full browser run recorded 48 passed and 10 responsive tests unable to launch. The Observatory qualification therefore remains **CONDITIONAL PASS**, with the existing blockers still active:
