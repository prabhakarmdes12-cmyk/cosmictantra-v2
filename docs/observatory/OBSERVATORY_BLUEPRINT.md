...
**Latest committed baseline:** `dea9217 docs: review guide for local agent (PR #1 verification checklist)`
**Current implementation slice:** synchronized study cockpit, target-aware snapshot/provenance workflow, live/reference imaging gateway, provider capability catalog, safety seam, and MCP control-plane integration
...
The next study-cockpit pass now makes the selected target the common key across calculation, reality and evidence:

- `StudyCockpit` presents local coordinates/model, provider-check state/frame metadata, city/UTC context, and the active qualification blockers in three visibly separate cards;
- `studyContext` gives planets, Rahu/Ketu, bright-star anchors, constellation patterns and reserved events target-specific local semantics rather than borrowing the selected graha's numbers;
- `studySnapshot` creates a versioned, browser-local envelope containing the exact city/instant, local calculation provenance, provider capabilities/notices, complete frame provenance when available, attribution/licensing notes, and `BLOCKER-1`/`BLOCKER-2` status;
- the notebook persists the snapshot in JSON and flattens its provenance into CSV, while old entries without a snapshot remain readable;
- direct route-handler integration tests cover target validation, default-deny hardware, stateless MCP initialization and CORS preflight.
...
The interaction diagram explains why canvas hits, accessible controls, and shared URLs all converge on `CelestialSelection` instead of implementing separate planet/star/constellation logic.
...
| “Can I save this as a study record?” | Synchronized Study Cockpit plus versioned browser-local snapshot containing city, instant, local model, provider check/frame provenance, licensing notes, qualification blockers, JSON export and flattened CSV export. |
...
| --- | --- | --- | --- |
| `/observatory` | Local sky view | City, city-local observation time, selected planet/star/constellation | Stereographic sky, planet rail, layers, constellation guide, bright-anchor list, Sky at a glance, Study Cockpit, detail sheet |
...
| `ObservatoryStudentDesk.tsx` | Integrates approximate field planning, Moon phase, coordinate study, selected-object guidance, synchronized study cockpit, local notebook, and curated optional study links into the primary route. |
| `StudyCockpit.tsx` | Shows the same target, city and instant as three separate cards: local calculated sky, provider reality/check, and qualification evidence. |
...
| `ObservationLog.tsx` | Saves a versioned city/time/target snapshot locally, keeps frame and local provenance together, distinguishes physical observations from node study notes, and exports JSON/CSV. |
...
| `src/lib/astronomy/celestialCatalog.ts` | Typed planet/star/constellation selections, explanatory copy, catalogue-anchor detail metadata, constellation names/stories, and deep-link validation. |
...
| `src/lib/astronomy/observationLog.ts` | Validated browser-local observation-log schema with persistence, versioned snapshot attachment, and CSV serialization. |
...
| `src/lib/observatory/studyContext.ts` | Target-aware local study context for canonical bodies/nodes, bright stars, constellation patterns and reserved events. |
| `src/lib/observatory/studySnapshot.ts` | Versioned snapshot envelope, provider-frame provenance copy, capability/notices capture, and fail-closed local-storage parsing. |
...
| File | Responsibility |
| --- | --- |
| `tests/observatory.spec.ts` | Observatory coordinate, body, boundary, artwork metadata, deep-link, viewport, observation-helper, Moon-phase, local study-context, and snapshot/provenance invariants. |
| `tests/observatory.integration.spec.ts` | Direct HTTP-handler integration checks for live target validation, locked hardware requests, stateless MCP initialization, and CORS preflight behavior. |
...
- **Study Cockpit:** synchronized target dossier showing local calculation semantics, provider reality/check state, city/UTC context, and the current qualification blockers.
- **Snapshot notebook:** the local save action bundles target-aware local context, provider capabilities/notices, frame provenance, attribution/licensing notes, and qualification state into a versioned JSON record; CSV exposes flattened provenance columns.
...
### 7.7 Synchronized Study Cockpit and snapshot workflow

`StudyCockpit` is the product-facing join between the existing local instruments and the live/reference seam. It receives the selected target, the same `Date` instant, the selected city's fixed observer coordinates, the target-aware `LocalStudyContext`, and the latest provider response. It renders three separate evidence cards:

1. **Calculated local sky** — source path, model, quality, frame, physical-versus-mathematical semantics, and only the coordinates that are meaningful for that target.
2. **Reality/provider** — frame provider, requested/captured/received times, wavelength/filter, quality/freshness and source metadata, or an explicit not-queried/no-frame state.
3. **Evidence status** — `CONDITIONAL PASS`, `BLOCKER-1`, `BLOCKER-2`, and the instruction that a provider image is not a coordinate overlay.

`createLocalStudyContext` deliberately differs by target type: a planet receives the existing canonical ephemeris and horizontal plan, a star receives the projected catalogue anchor, a constellation receives a pattern-only context without a fabricated centre, and Rahu/Ketu receive longitude-only node semantics. This prevents the common UI error where selecting a star or pattern leaves the previous graha's altitude and rashi numbers attached to the record.

The notebook save action creates an `ObservatoryStudySnapshot` with a versioned schema. It contains the exact selected instant, city/observer context, local calculation provenance, provider capability check and notices, a complete copied frame metadata record when one exists, attribution/licensing/use notes, transport pointers, and the active qualification blockers. It is stored in the existing browser-local log; old entries without the optional snapshot envelope remain readable. JSON preserves the nested record, while CSV adds flattened provenance columns for classroom or spreadsheet handoff. No image bytes are stored in the snapshot.
...
### Step 13 — synchronized study cockpit and evidence-backed snapshots

The study-workspace pass joins the local instruments and external reality layer without replacing either:

1. Added `createLocalStudyContext` for target-specific planet/node, bright-star, constellation-pattern and reserved-event semantics.
2. Added `StudyCockpit` with separate local-calculation, provider-reality and qualification cards driven by the same target/city/instant.
3. Connected `LiveObservationPanel` to the parent experience so the cockpit and notebook receive the current provider response; target, date, and zoom-gate changes clear stale provider state.
4. Added versioned `ObservatoryStudySnapshot` creation and fail-closed parsing with city/observer context, local model/frame, provider capabilities/notices, frame metadata, rights notes and active qualification blockers.
5. Extended the browser-local notebook to persist snapshots, reopen star/constellation targets, display provenance status, and export flattened provenance fields in CSV while retaining nested JSON.
6. Added direct HTTP-handler integration coverage for live target validation, locked hardware requests, stateless MCP initialization and CORS preflight behavior.
7. Re-ran the qualification suite: 30 local Observatory invariants, 5 Observatory HTTP-boundary integration tests, and 13 existing engine tests pass; the full browser suite remains limited by the missing Chromium executable.
...
### Decision P — save evidence bundles, not just coordinates

**Choice:** make a snapshot an immutable, versioned copy of local calculation context plus the provider check/frame provenance at the selected city and instant.
**Why:** a naked longitude or an image URL is not reproducible evidence. The learner needs to know which target was selected, which coordinate frame/model was used, whether a provider was queried, what was captured/received, and which qualification limits still apply.
**Trade-off:** JSON records are larger and CSV needs flattened columns, but the browser-local workflow stays portable and no cloud account or image-byte upload is introduced. Old notebook entries remain readable through the optional snapshot field.
...
```bash
npx playwright test tests/observatory.spec.ts tests/observatory.integration.spec.ts --reporter=line
```
...
1. **Prisma generation/build is network-blocked** because the Prisma engine checksum request cannot establish TLS in this sandbox. `npm run build` therefore remains unqualified here.
...
The current validation split is **30 local Observatory invariants + 5 HTTP-boundary integration tests + 13 engine tests passed**. The full `npm test` run recorded 48 passed and 10 responsive/browser tests unable to launch solely because the Chromium executable is unavailable.
...
- [ ] Click a visible star/line and confirm the correct catalogue-star or constellation sheet.
- [ ] Reopen `/observatory?object=sirius&objectKind=star` and confirm Sirius remains the selected target with local altitude/azimuth and no fabricated astrological coordinates.
