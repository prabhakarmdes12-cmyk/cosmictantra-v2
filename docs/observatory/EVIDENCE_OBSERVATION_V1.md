# Observatory implementation note — Evidence-backed Observation + Student Desk v2
...
- [`StudyCockpit.tsx`](../../src/components/observatory/StudyCockpit.tsx) adds a synchronized target dossier for the local calculation, provider reality state, and qualification evidence. It is driven by the same target, city, and instant as the sky and never overlays unlike frames.
- [`studyContext.ts`](../../src/lib/observatory/studyContext.ts) derives target-aware local context for planets, Rahu/Ketu, bright stars, constellations, and reserved events. Unsupported targets retain an explicit no-coordinate state instead of borrowing a different body's values.
- [`studySnapshot.ts`](../../src/lib/observatory/studySnapshot.ts) defines a versioned snapshot envelope containing local calculation provenance, provider capabilities/notices, complete external-frame metadata when available, city/instant context, and the active `CONDITIONAL PASS` blockers.
...
[`ObservationLog.tsx`](../../src/components/observatory/ObservationLog.tsx) records the selected city, instant, target, local coordinates, tropical/sidereal context, observation status, and optional field note in browser `localStorage`. New saves carry a versioned `ObservatoryStudySnapshot` containing the target-aware local context, provider-check state, frame provenance, attribution/licensing notes, transport pointers, and `CONDITIONAL PASS` blockers. JSON includes the full snapshot and CSV adds flattened provenance columns for classroom/spreadsheet handoff. Stored entries validate against [`observationLog.ts`](../../src/lib/astronomy/observationLog.ts) and [`studySnapshot.ts`](../../src/lib/observatory/studySnapshot.ts); malformed records are discarded rather than rendered as trusted measurements. A Rahu/Ketu entry is labelled a mathematical-node study note and contains no physical altitude or azimuth.

### A synchronized study cockpit

[`StudyCockpit.tsx`](../../src/components/observatory/StudyCockpit.tsx) makes the selected target the common key for three adjacent evidence layers: the calculated local sky, the optional provider frame/check, and the qualification state. [`createLocalStudyContext`](../../src/lib/observatory/studyContext.ts) handles planets, mathematical nodes, bright-star anchors, constellation patterns and future event ids separately. A star receives local horizontal coordinates but no invented rashi; a constellation receives a pattern note but no fabricated single centre coordinate; Rahu/Ketu retain longitude-only semantics. This keeps the study experience rich without smuggling a selected graha's numbers into a different target.

The cockpit also keeps the exact UTC instant, city name and observer coordinates visible next to the cards. A frame is described by its provider/capture/receive metadata and source link, while the local calculation remains a separate record. The provider response is cleared when the target, instant, or deep-zoom gate changes, so a saved snapshot cannot accidentally combine a previous frame with a new local context.
...
- [`src/components/observatory/ObservationLog.tsx`](../../src/components/observatory/ObservationLog.tsx) — browser-local observation/study notebook with target-aware snapshots, reopen links and JSON/CSV export.
- [`src/components/observatory/StudyCockpit.tsx`](../../src/components/observatory/StudyCockpit.tsx) — synchronized local/reality/evidence dossier for the selected target.
- [`src/lib/observatory/studyContext.ts`](../../src/lib/observatory/studyContext.ts) — target-aware local calculation context for planets, nodes, stars, constellations and reserved events.
- [`src/lib/observatory/studySnapshot.ts`](../../src/lib/observatory/studySnapshot.ts) — versioned snapshot creation, frame-provenance copying and fail-closed parsing.
...
- [`tests/observatory.spec.ts`](../../tests/observatory.spec.ts) — astronomy invariants plus viewport, observation-helper, snapshot/provenance and Moon-phase tests.
- [`tests/observatory.integration.spec.ts`](../../tests/observatory.integration.spec.ts) — direct HTTP-boundary coverage for live target validation, locked hardware actions, MCP transport and CORS preflight.
...
- `npx playwright test tests/observatory.spec.ts --reporter=line` — **30 passed**. This includes canonical astronomy invariants, progressive zoom-detail tier determinism, Rahu/Ketu node semantics, viewport focus/clamping behavior, local solar event signals, altitude/direction helpers, display-only horizon/magnitude filters, bounded Moon phase output, sampled horizon-crossing and angular-separation planner signals, local observation-log round trips/CSV escaping, target-aware study context, versioned snapshot round trips, the local provider adapter, live target/provider/frame metadata boundaries, server-side URL construction, fail-closed hardware safety, validated Alpaca/INDI agent configuration, and fail-closed reference-fixture validation.
- `npx playwright test tests/observatory.integration.spec.ts --reporter=line` — **5 passed**. This exercises direct Next HTTP route handlers for local-only target responses, invalid-target rejection, locked hardware requests, stateless MCP initialization, and CORS preflight/session behavior.
- `npm run typecheck` — **passed** with no TypeScript errors.
- `npm run test:engines -- --reporter=line` — **13 passed**.
- `npm run dev -- --hostname 0.0.0.0` plus HTTP checks — `/observatory` returned HTTP 200 with the synchronized study cockpit and snapshot notebook rendered in the server response; the other Observatory routes remain covered by the prior smoke record. External Sun imagery remains network-dependent.
- `npm run build` — **blocked before Next compilation by Prisma engine checksum retrieval** because `binaries.prisma.sh` was unavailable in this sandbox. This is an environment/setup limitation, not a reported Observatory type error.
- Full `npm test` — **48 passed, 10 failed only because Chromium is not installed** for `tests/responsive.spec.ts`. Full browser interaction/responsive QA remains dependent on Chromium availability. The qualification guardrail remains **CONDITIONAL PASS**.
