...
- `StudyCockpit.tsx` now joins the same target/city/instant across local calculation, provider reality/check state and qualification evidence without overlaying unlike frames.
- `studyContext.ts` preserves distinct semantics for canonical planets, Rahu/Ketu, bright-star anchors, constellation patterns and reserved events.
- `studySnapshot.ts` and `ObservationLog.tsx` persist a versioned, browser-local evidence bundle with provider notices, complete frame provenance, attribution/licensing notes, JSON export and flattened CSV columns; older entries remain readable.
- `tests/observatory.integration.spec.ts` directly exercises the live, hardware-safety and MCP HTTP boundaries.
...
This remains deliberately capability-aware rather than a promise that every target has a free live feed. With the study cockpit now in place, the next implementation should add one provider at a time with provider permission, queue/freshness state, rights records, and browser-independent fallback behavior.

---

## Final recommendation

Do not start by filling the Observatory with hundreds of remote images or by adding a cinematic 3D solar system. The local observation is now zoomable, reference-aware, and provenance-complete at the study-record level: the Study Cockpit keeps calculation, provider reality and qualification evidence separate, while the snapshot notebook makes the selected city/instant/target reproducible offline.

The next earned leap is evidence and provider depth, not visual replacement: complete a reviewed JPL Horizons fixture and Moon tolerance policy, perform Chromium QA, then add one permitted remote-exposure or local-agent adapter with queue/freshness/rights metadata. That is the foundation on which NASA/ISRO/Roscosmos media, planetary surface tiles, 3D models, AR/VR, mission theatre, and a living archive can become world-class rather than merely impressive.
